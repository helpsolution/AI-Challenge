...
description = "MCP-сервер сервиса пользователей: два инструмента поверх его REST API"
...
application {
    mainClass.set("advent.users.mcp.MainKt")
}
...
dependencies {
    ...
    // клиент: ходим в сервис пользователей, не блокируя поток корутины
...
tasks.jar {
    manifest {
        attributes["Main-Class"] = "advent.users.mcp.MainKt"
    }
