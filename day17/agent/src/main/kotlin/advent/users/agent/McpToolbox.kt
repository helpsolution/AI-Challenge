...
class McpToolbox private constructor(
    ...
    suspend fun call(toolCall: ToolCall): ToolOutcome {
        ...
        val result = try {
            client.callTool(
                CallToolRequest(CallToolRequestParams(name = toolCall.function.name, arguments = arguments)),
            )
        } catch (e: Exception) {
            // Сбой инструмента не должен ронять диалог: отдаём модели причину, она попробует иначе.
            return ToolOutcome(
                text = "Инструмент ${toolCall.function.name} не выполнен: ${e.message ?: e::class.simpleName}",
                structured = null,
                isError = true,
            )
        }
