...
class ChatController(
    ...
    fun invariants() = advent.day14.invariant.InvariantPolicy.DEFAULT
    ...
    fun acceptStyleSuggestion(@PathVariable id: Long): advent.day14.memory.MemoryItem {
    ...
    fun longTerm(): List<advent.day14.memory.MemoryItem> =
        longTerm.list(agent.properties.longTermLimit)

    @PostMapping("/memory/long-term")
    fun upsertLongTerm(@RequestBody request: UpsertMemoryRequest): advent.day14.memory.MemoryItem =
        longTerm.upsert(
            NewMemoryItem(
                kind = request.kind,
