...
import advent.day14.chat.Exchange
...
import advent.day14.llm.ChatCompletionRequest
import advent.day14.llm.LlmClient
import advent.day14.llm.LlmException
import advent.day14.llm.ResponseFormat
...
class Agent(
    ...
    private val invariantGuard: advent.day14.invariant.InvariantGuard = advent.day14.invariant.InvariantGuard(),
) {
    ...
    fun ask(sessionId: Long, question: String): AgentExchange {
        ...
        try {
            ...
            val candidate = memoryRouter.preview(sessionId, decision)
            val verdict = invariantGuard.check(decision, candidate)
            val reply = if (verdict.allowed) proposedReply else verdict.reason
            trace = trace?.let { inspection?.event(it.copy(decision = decision, invariantVerdict = verdict, deliveredReply = reply),
                "guard", if (verdict.allowed) "Инварианты соблюдены" else "Запрос отклонен: " + verdict.violations.joinToString()) }
            if (verdict.allowed) {
            ...
            val routing = if (verdict.allowed) memoryRouter.commit(text, candidate)
                else MemoryRoutingResult(working.get(sessionId), emptyList())
            trace = trace?.let { inspection?.event(it, "output", if (verdict.allowed) "Ответ готов, память сохранена" else "Отказ сохранен, состояние поста не изменено") }
            trace?.let { inspection?.finish(it) }
