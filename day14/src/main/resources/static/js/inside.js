...
const sourceNames = {invariants:'Неизменяемые инварианты', persona:'Роль редактора', profile:'Профиль', rules:'Формат и правила ответа', longTerm:'Долговременные правила', working:'Рабочая память', shortTerm:'Окно диалога', input:'Новое сообщение'};
const eventNames = {guard:'Проверка инвариантов', input:'Вход', profileSelect:'Выбор профиля', prompt:'Контекст', llm:'LLM-клиент', parser:'Разбор JSON', router:'Запись памяти', output:'Готово'};
const statuses = {REFUSED:'Отказ по инвариантам', RUNNING:'В работе', SUCCESS:'Завершён', ERROR:'Ошибка', INTERRUPTED:'Прерван'};
...
async function chooseSession(id) {
  ...
  localStorage.setItem('day14-session', String(sessionId));
...
function render() {
  ...
  $('guard-content').innerHTML = verdict ? `<p><strong>${verdict.allowed ? 'Проверка пройдена' : 'Отказ · память не изменена'}</strong></p><p class="quiet">${esc(verdict.violations.join(', ') || verdict.reason)}</p>` : '<p>Проверяет ответ и будущее состояние до записи</p>';
  ...
  $('router-content').innerHTML = memoryChanges.length ? `<div class="tag-row">${memoryChanges.slice(0, 4).map(c => tag(`${fields[c.field]} ${c.after ? '+' : '−'}`, 'green')).join('')}</div>` : `<p>${trace?.events.some(e => e.node === 'router') && !running ? 'Изменений рабочей и долгой памяти нет' : trace?.invariantVerdict?.allowed === false ? 'Пропущен: инварианты не пройдены' : 'Применяет изменения и проверяет этап'}</p><p class="quiet">Правило стиля сохраняется после подтверждения</p>`;
...
function drawConnections() {
  ...
  const resultNode = trace?.invariantVerdict?.allowed === false ? box('guard') : router;
...
function detail(id) {
  ...
  if (id === 'guard') body = section('Вывод модели', pre(trace?.decision?.invariantCheck || 'Пока нет')) + section('Проверка кодом', pre(trace?.invariantVerdict || 'Пока нет')) + paragraph('Длина ответа и черновика, хештеги во всех предлагаемых полях. При отказе сохраняется только переписка, пост и стиль остаются прежними.');
...
sessionId = Number(new URLSearchParams(location.search).get('session') || localStorage.getItem('day14-session')) || null;
