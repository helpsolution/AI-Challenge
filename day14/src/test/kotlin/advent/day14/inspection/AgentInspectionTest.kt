package advent.day14.inspection

import advent.day14.agent.*
import advent.day14.config.AgentProperties
import advent.day14.llm.*
import advent.day14.memory.MemoryKind
import advent.day14.memory.NewMemoryItem
import advent.day14.store.SqliteMemoryStore
...
class AgentInspectionTest {
    ...
    fun `trace preserves exact request old context usage and actual memory changes`() = Fixture().use { f ->
        val reply = """{"invariantCheck":{"checkedIds":["SHORT_TEXT","NO_HASHTAGS"],"conflictIds":[],"explanation":"Соответствует правилам"},"reply":"Сформулируем тезис","taskUpdate":{"stage":"THESIS","idea":"Практика AI"},"styleSuggestion":{"key":"Тон","value":"Спокойный"}}"""
        ...
        assertEquals(listOf("input", "prompt", "llm", "parser", "guard", "router", "output"), trace.events.map { it.node })
    ...
    fun `pending call is observable before completion and deleted session removes traces only`() = Fixture().use { f ->
        ...
        try {
            val agent = f.agent {
                ...
                LlmCompletion("""{"invariantCheck":{"checkedIds":["SHORT_TEXT","NO_HASHTAGS"],"conflictIds":[],"explanation":"Соответствует правилам"},"reply":"Готово"}""")
            }
