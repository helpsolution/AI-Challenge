...
const fields = {profile:'Профиль', messages:'Сообщения', state:'Этап', currentStep:'Текущий шаг', expectedAction:'Ожидаемое действие', idea:'Идея', thesis:'Тезис', plan:'План', draft:'Черновик', notes:'Замечания', styleSuggestion:'Предложение правила', longTerm:'Долговременные правила'};
...
async function chooseSession(id) {
  ...
  localStorage.setItem('day13-session', String(sessionId));
...
function render() {
  ...
  $('working-content').innerHTML = `<div class="tag-row">${tag(stages[task?.state] || 'Идея', 'amber')}${task?.plan?.length ? tag(`${task.plan.length} пункта плана`, 'muted') : ''}${task?.draft ? tag('Есть черновик', 'muted') : ''}</div><p class="line-clamp" style="margin-top:5px"><strong>${esc(task?.currentStep || 'Уточнение идеи')}</strong><br>${esc(short(task?.expectedAction || 'Пользователь описывает тему и замысел поста', 115))}</p>${task?.styleSuggestion ? '<p class="quiet">+ правило ожидает подтверждения</p>' : ''}`;
...
function detail(id) {
  ...
  if (id === 'working') body = '<p class="detail-note">SQLite → post_task. Одно рабочее состояние на сессию.</p>' + section('Этап', paragraph(stages[task?.state])) + ['currentStep','expectedAction','idea','thesis','plan','draft','notes','styleSuggestion'].map(key => section(fields[key], paragraph(Array.isArray(task?.[key]) ? task[key].join('\n') : key === 'styleSuggestion' && task?.styleSuggestion ? `${task.styleSuggestion.key}: ${task.styleSuggestion.value}` : task?.[key]))).join('');
...
sessionId = Number(new URLSearchParams(location.search).get('session') || localStorage.getItem('day13-session')) || null;
