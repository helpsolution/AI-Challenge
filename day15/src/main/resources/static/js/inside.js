...
const stages = {IDEA:'Идея', THESIS:'Тезис', PLAN:'План', DRAFT:'Черновик', VALIDATION:'Проверка', DONE:'Готово'};
const names = {invariants:'Инварианты канала', guard:'Контроль перехода', profile:'Профиль', shortTerm:'Краткосрочная память', working:'Рабочая память', longTerm:'Долговременная память', entry:'Обработка сообщения', prompt:'Сборщик промпта', parser:'Разбор ответа', router:'Маршрутизатор памяти', llm:'LLM-клиент', model:'Модель', output:'Результат шага'};
...
const sourceNames = {invariants:'Неизменяемые инварианты', lifecycle:'Карта переходов', persona:'Роль редактора', profile:'Профиль', rules:'Формат и правила ответа', longTerm:'Долговременные правила', working:'Рабочая память', shortTerm:'Окно диалога', input:'Новое сообщение'};
const eventNames = {transition:'Проверка перехода', guard:'Проверка инвариантов', input:'Вход', profileSelect:'Выбор профиля', prompt:'Контекст', llm:'LLM-клиент', parser:'Разбор JSON', router:'Запись памяти', output:'Готово'};
const statuses = {TRANSITION_REFUSED:'Недопустимый переход', REFUSED:'Отказ по инвариантам', RUNNING:'В работе', SUCCESS:'Завершён', ERROR:'Ошибка', INTERRUPTED:'Прерван'};
...
async function chooseSession(id) {
  ...
  localStorage.setItem('day15-session', String(sessionId));
...
function render() {
  ...
  $('invariants-content').innerHTML = (view.invariants?.rules || []).map(r => `<p class="rule-line">${esc(r.description)}</p>`).join('');
  const verdict = trace?.invariantVerdict;
  const transition = trace?.transitionVerdict;
  $('guard-content').innerHTML = transition
    ? `<p><strong>${transition.allowed ? `${esc(stages[transition.from])} → ${esc(stages[transition.target])}` : 'Переход запрещен'}</strong></p><p class="quiet">${esc(transition.reason)}</p>${verdict ? `<p class="quiet">Инварианты: ${verdict.allowed ? 'соблюдены' : esc(verdict.violations.join(', '))}</p>` : ''}`
    : '<p>Проверяет переход и будущее состояние до записи</p>';
  ...
  $('router-content').innerHTML = memoryChanges.length ? `<div class="tag-row">${memoryChanges.slice(0, 4).map(c => tag(`${fields[c.field]} ${c.after ? '+' : '−'}`, 'green')).join('')}</div>` : `<p>${trace?.events.some(e => e.node === 'router') && !running ? 'Изменений рабочей и долгой памяти нет' : trace?.transitionVerdict?.allowed === false ? 'Пропущен: переход запрещен' : trace?.invariantVerdict?.allowed === false ? 'Пропущен: инварианты не пройдены' : 'Сохраняет проверенный кандидат'}</p><p class="quiet">Правило стиля сохраняется после подтверждения</p>`;
  ...
  $('output-content').innerHTML = trace?.error ? `<p class="line-clamp">${esc(trace.error)}</p>` : running ? '<p>Шаг выполняется…</p>' : trace ? `<p><strong>${trace.transitionVerdict?.allowed === false ? 'Недопустимый переход' : trace.invariantVerdict?.allowed === false ? 'Отказ по инвариантам' : isProfile ? s.profile ? 'Профиль выбран' : 'Нужно уточнить профиль' : trace.kind === 'MEMORY' ? 'Память обработана' : 'Ответ сохранён в чате'}</strong></p><p class="quiet">${memoryChanges.length ? `Изменено полей памяти: ${memoryChanges.length}` : 'Без изменений полей памяти'} · ${duration(trace)}</p>` : '<p>Здесь появится итог действия</p><p class="quiet">Полный ответ доступен в редакторе</p>';
  ...
  if (prompt) ['invariants','profile','working','shortTerm', ...(prompt.snapshot.longTerm.length ? ['longTerm'] : [])].forEach(id => mark(id, 'visited'));
...
function drawConnections() {
  ...
  ['invariants','profile','shortTerm','working','longTerm'].forEach((id, i) => {
  ...
  const resultNode = trace?.transitionVerdict?.allowed === false || trace?.invariantVerdict?.allowed === false ? box('guard') : router;
  line(`M ${resultNode.right} ${resultNode.y} H ${lane} V ${output.y} H ${output.left}`, trace?.events.some(e=>e.node==='output') && !trace.events.some(e=>e.node==='profileSelect') ? 'used-path' : '');
...
function detail(id) {
  ...
  $('detail-subtitle').textContent = mode === 'step' ? `СНИМОК ШАГА · ${time(trace?.startedAt)}` : ['invariants','profile','shortTerm','working','longTerm'].includes(id) ? 'ТЕКУЩЕЕ СОСТОЯНИЕ' : 'ПОСЛЕДНЕЕ ЗАПИСАННОЕ ДЕЙСТВИЕ';
  let body = '';
  if (id === 'invariants') body = section('Правила из отдельного файла', pre(view.invariants)) + paragraph('Загружаются при старте из invariants.json. История, профиль и правила стиля не могут изменить этот файл.');
  if (id === 'guard') body = section('Предложенный переход', pre(trace?.decision?.transition || 'Переход не предложен')) + section('Вердикт жизненного цикла', pre(trace?.transitionVerdict || 'Пока нет')) + section('Вывод модели об инвариантах', pre(trace?.decision?.invariantCheck || 'Пока нет')) + section('Проверка инвариантов кодом', pre(trace?.invariantVerdict || 'Не запускалась')) + paragraph('Сначала проверяются разрешенный переход и данные этапа, затем инварианты. При отказе сохраняется только переписка.');
  ...
  if (id === 'output') body = section('Статус', paragraph(trace ? statuses[trace.status] : 'Действий пока нет')) + (trace?.decision ? section('Ответ пользователю', paragraph(trace.deliveredReply || trace.decision.reply)) : '') + (trace?.error ? section('Ошибка', paragraph(trace.error)) : '') + changesDetail();
...
sessionId = Number(new URLSearchParams(location.search).get('session') || localStorage.getItem('day15-session')) || null;
