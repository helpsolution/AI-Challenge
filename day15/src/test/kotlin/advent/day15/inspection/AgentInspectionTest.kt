package advent.day15.inspection

import advent.day15.agent.*
import advent.day15.config.AgentProperties
import advent.day15.llm.*
...
class AgentInspectionTest {
    ...
    fun `trace preserves exact request old context usage and actual memory changes`() = Fixture().use { f ->
        val reply = """{"invariantCheck":{"checkedIds":["SHORT_TEXT","NO_HASHTAGS"],"conflictIds":[],"explanation":"Соответствует правилам"},"reply":"Сформулируем тезис","taskUpdate":{"idea":"Практика AI"},"transition":{"target":"THESIS"},"styleSuggestion":{"key":"Тон","value":"Спокойный"}}"""
        ...
        assertEquals(listOf("input", "prompt", "llm", "parser", "transition", "guard", "router", "output"), trace.events.map { it.node })
