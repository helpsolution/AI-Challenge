...
import advent.news.mcp.ServerConfig.Companion.MCP_PATH
...
fun main() {
    ...
    val log = LoggerFactory.getLogger("advent.news.mcp")

    log.info("MCP-сервер новостей: порт {}, сервис новостей {}", config.port, config.newsApiUrl)
...
fun Application.newsMcpModule(config: ServerConfig) {
    ...
    mcpStatelessStreamableHttp(
        ...
        Server(
            serverInfo = Implementation(name = "news-mcp", version = "1.0.0"),
...
/**
 * Токен — единственная преграда перед инструментами: subscribe_* и unsubscribe меняют данные,
 * и открытый наружу эндпоинт без проверки доступа означает, что подписки заводит и удаляет кто угодно.
