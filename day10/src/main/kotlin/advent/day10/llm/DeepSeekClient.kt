...
import advent.day10.config.DeepSeekProperties
import org.slf4j.LoggerFactory
import tools.jackson.databind.ObjectMapper
import java.net.URI

/**
 * Клиент DeepSeek — единственный провайдер этого дня.
 *
 * По части токенов у него две особенности, и обе прямо влияют на сравнение стратегий.
 *
 * Первая: кэш префикса он показывает плоским полем `prompt_cache_hit_tokens`. Неизменное
 * начало промпта стоит примерно в тридцать раз дешевле нового текста — а значит стратегия,
 * которая каждый ход перекраивает начало промпта, платит больше при том же числе токенов.
 * Без этого поля вывод «окно дешевле» получился бы правдоподобным и неверным.
 *
 * Вторая: денег DeepSeek не сообщает вообще, поэтому цену считаем сами по прайсу из конфига
 * и честно помечаем как оценку — прайс лежит в чужих руках и меняется, а тариф вне пиковых
 * часов ещё и вдвое ниже.
 */
